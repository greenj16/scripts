...
tmux send-keys -t my_session.0 'display_proc() { while true; do clear; count=0; while IFS= read -r line; do if [ $count -eq 0 ]; then echo "PID, PPID, CMD"; ((count++)); continue; else echo "${count}: ${line}"; fi; ((count++)); sleep 2; done < "/tmp/new_processes"; done; }' C-m
